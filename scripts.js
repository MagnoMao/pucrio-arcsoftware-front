...
const renderUsuarios = () => {
  document.getElementById('paginaLivros').style.display = "none"
  document.getElementById('paginaUsuarios').style.display = "block"
  document.getElementById('formularioUsuarioNovo').style.display = "none"
  document.getElementById('listagemUsuarios').style.display = "flex"
  getListaUsuarios()
}

const renderLivros = () => {
  document.getElementById('paginaLivros').style.display = "block"
  document.getElementById('formularioUsuarioNovo').style.display = "none"
...
const formularioUsuarioNovo = () => {
  document.getElementById('formularioUsuarioNovo').style.display = "flex"
  document.getElementById('listagemUsuarios').style.display = "none"
}


/*
  --------------------------------------------------------------------------------------
  Função para consultar CEP via api ViaCEP
  --------------------------------------------------------------------------------------
*/

const consultaCEP = async () => {
  let cepElem = document.getElementById('cep')

  // Garante que o usuário consiga digitar apenas dígitos
  cepElem.value = cepElem.value.replace(/\D/g, "");
  //errorElem.innerHTML = "texto do aviso aqui"
  document.getElementById('logradouro').value = ''
  document.getElementById('bairro').value = ''
  document.getElementById('uf').value = ''

  if(cepElem.value.length != 8) return
  let url = 'https://viacep.com.br/ws/' + cepElem.value + '/json/'
...
const postEndereco = () => {
  ...
  let complememento = document.getElementById('complememento')
  ...
  formData.append('complememento', complememento);
  formData.append('endereco_de', '');
  ...
  .then((response) => {
    ...
    res.then(msg =>{
      if (msg.mensagem) {
        alert(msg.mensagem) // Houve um erro, endereco não adicionado
      }
    })
  })
  .catch((error) => {
